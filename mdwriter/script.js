class MarkdownEditor {
    ...
    initEventListeners() {
        ...
        if (this.ipcRenderer) {
            ...
            // 监听文件夹打开结果
            this.ipcRenderer.on('folder-opened', (event, data) => {
                if (data.error) {
                    this.showToast(`读取文件夹失败: ${data.error}`);
                    return;
                }
                this.currentFolder = data.folderPath;
                this.renderFileList(data.files);
                this.showToast(`已加载 ${data.files.length} 个文件`);
            });
            
            // 监听文件读取结果
            this.ipcRenderer.on('file-read', (event, data) => {
                if (data.error) {
                    this.showToast(`读取文件失败: ${data.error}`);
                    return;
                }
                this.editor.value = data.content;
                this.currentFile = data.filePath;
    ...
    openFolder() {
        if (this.isElectron && this.ipcRenderer) {
            ...
            this.showToast('文件夹浏览功能需要在Electron环境中使用');
        }
    }
    
    renderFileList(files) {
        if (files.length === 0) {
            this.fileList.innerHTML = '<div class="toc-empty">该文件夹中没有 .md 文件</div>';
            return;
        }
        
        let html = '<ul class="file-list">';
        files.forEach((file, index) => {
            const isActive = this.currentFile === file.path ? ' active' : '';
            html += `<li class="file-item${isActive}" data-path="${this.escapeHtml(file.path)}" title="${this.escapeHtml(file.path)}">${this.escapeHtml(file.name)}</li>`;
        });
        html += '</ul>';
        ...
        this.fileList.querySelectorAll('.file-item').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                this.openFileFromList(filePath);
                // 高亮当前选中
    ...
    openFileFromList(filePath) {
        if (this.isElectron && this.ipcRenderer) {
            this.ipcRenderer.send('read-file', filePath);
        }
    }
